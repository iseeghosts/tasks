...
export default class SavedNote extends Component{
    ...
    constructor (props) {
        super()
            this.state = ({
                ...
                Delete_Task:props.task.deleted?Restore_Task:Delete_Task_Dark,
                permdisp:true,
                backColor:(props.task.status=="Incomplete"? '#555555':'steelblue'),                                        // 
                backColorTaskBox:(props.task.content==''? '#777777':'transparent'),
    ...
    change_status = () => {
        this.setState({
                ...
                'backColor': (this.state.Task_Status==Task_Done ? '#555555':'steelblue'),
            })
    ...
    delete_task = () => {
        ...
        if (this.state.task) {
            ...
            this.setState({deletedTask:false, Delete_Task:Delete_Task_Dark})
        }
    }
    ...
    edit_task = () => {
        var date = new Date().toLocaleString();
            this.setState({
                markDoneDisable: (this.state.Edit_Status==Edit_Task),
                Edit_Status: (this.state.Edit_Status==Done_Editing? Edit_Task:Done_Editing),
                backColorTaskBox: (this.state.Edit_Status==Done_Editing ? 'transparent':'#777777'),
                backColor: (this.state.task==''? '#AAAAAA':'#555555'),
            })
    ...
    render () {   
        ...
        var mainbox = StyleSheet.flatten([
            styles.maincontainer,{
                ...
                marginVertical:AppData[this.props.id].taskMargin,
            ...
            {/* main Container containing all children items */}
                <View style={mainbox2}>
                    ...
                    <View style={styles.taskstatusbox}>
                        <TouchableHighlight disabled={this.state.task==''|| this.state.Edit_Status==Done_Editing || this.state.deletedTask} onPress={() => this.change_status()} style={styles.taskdone}>
                            <Image source={this.state.Task_Status} style={styles.logo} /> 
                        </TouchableHighlight>
                    </View>

                    <View style={styles.infobox}>
                        <TouchableHighlight underlayColor='white' onPress={() => this.task_details()} style={styles.detailsbutton}>
                    ...
                    <View style={inputtask}>
                        <TextInput onSubmitEditing={()=> {if (this.state.task!='') this.edit_task()}} keyboardAppearance={AppData[this.props.id].theme} style={{color:'white', fontSize:15, fontStyle:'italic', paddingVertical:10}}
                        editable={this.state.Edit_Status==Done_Editing && !deleted} defaultValue={this.state.task} placeholder={''} onChangeText={(task) => this.setState({task})} />
                    </View>

                    {/* {child} this button allows the task to be modified [edit_task], has a variable image uri Edit_Status */}
                    <TouchableOpacity disabled={this.state.Task_Status==Task_Done || this.state.deletedTask || content==''} underlayColor="#000"  style={styles.edittextbox} onPress={ () => this.edit_task()}>
                    ...
                    <TouchableOpacity disabled={this.state.Edit_Status==Done_Editing && !(content=='')} underlayColor="#000"  style={[styles.deletetask, {backgroundColor:this.state.deletedTask?'#004400':'darkred', borderBottomRightRadius:found && search.includes('@deleted')?0:5}]} onPress={ () => this.delete_task()}>
